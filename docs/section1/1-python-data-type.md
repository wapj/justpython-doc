...
쌍따옴표 3개(""") 로 감싸져 있는것은 문자열인데요. 개행을 별도로 표기하지 않고 싶을 때 사용합니다. 그리고 `f`는 f-string 이라는 기능을 사용하기 위해서 붙입니다. 변수를 문자열 내부에 중괄호 `{}` 로 감싸서 넣기 위해서 사용합니다.
